...
/*   Updated: 2022/04/27 22:39:07 by vmiseiki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_split(char const *str, char c)
{
	...
	if (!str)
		return (NULL);
	arr = (char **) malloc((ft_count_mem_for_arr(str, c) + 1) * sizeof(char *));
	if (!arr)
		return (0);
	i = 0;
	j = 0;
	while (j < ft_count_mem_for_arr(str, c))
	{
		if (str[i] != c && str[i] != '\0')
		{
			strlen = ft_count_str_char(str, c, i);
			arr[j] = ft_sub_arr(str, c, i, strlen);
			i = i + strlen;
			j++;
		}
		i++;
